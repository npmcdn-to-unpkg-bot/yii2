...
class Internet extends \Faker\Provider\Base
{
    ...
    protected static $loginFormats = array(
        '{{lastName}}.{{firstName}}',
        '{{firstName}}.{{lastName}}',
        '{{firstName}}##',
        '?{{lastName}}',
    ...
    {
        return preg_replace('/\s/u', '', $this->login() . '@' . static::safeEmailDomain());
    }
    ...
    {
        return preg_replace('/\s/u', '', $this->login() . '@' . static::freeEmailDomain());
    }
    ...
    {
        return preg_replace('/\s/u', '', $this->login() . '@' . $this->domainName());
    }
    ...
    {
        $format = static::randomElement(static::$loginFormats);
        $login = static::bothify($this->generator->parse($format));

        return strtolower(static::transliterate($login));
    }
    /**
     * @example 'fY4èHdZv68'
     */
