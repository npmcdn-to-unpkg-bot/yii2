...
class Swift_Transport_Esmtp_AuthHandler implements Swift_Transport_EsmtpHandler
{
    ...
    /**
     * Set the login to authenticate with.
     *
     * @param string $login
     */
    ...
    /**
     * Get the login to authenticate with.
     *
     * @return string
     */
        ...
        if ($this->_login) {
            ...
            foreach ($this->_getAuthenticatorsForAgent() as $authenticator) {
                if (in_array(strtolower($authenticator->getAuthKeyword()),
                    ...
                    ++$count;
                    if ($authenticator->authenticate($agent, $this->_login, $this->_password)) {
                        return;
                    }
                }
            }
            throw new Swift_TransportException(
                'Failed to authenticate on SMTP server with login "'.
                $this->_login.'" using '.$count.' possible authenticators'
                );
        }
    }
    ...
    {
        return array('setlogin', 'getlogin', 'setPassword', 'getPassword', 'setAuthMode', 'getAuthMode');
    }
