...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements Swift_Transport_Esmtp_Authenticator
{
     ...
     * @return bool
     */
    public function authenticate(Swift_Transport_SmtpAgent $agent, $login, $password)
    {
        ...
        try {
            ...
            // Message 3 response
            $this->sendMessage3($response, $login, $password, $timestamp, $client, $agent);
     ...
     * @return string
     */
    protected function sendMessage3($response, $login, $password, $timestamp, $client, Swift_Transport_SmtpAgent $agent, $v2 = true)
    {
        list($domain, $login) = $this->getDomainAndlogin($login);
        ...
        if (!$v2) {
            ...
            // LMv2
            $lmResponse = $this->createLMv2Password($password, $login, $domain, $challenge, $client);
            // NTLMv2
            $ntlmResponse = $this->createNTLMv2Hash($password, $login, $domain, $challenge, $blob, $timestamp, $client);
        }

        $message = $this->createMessage3($domain, $login, $workstation, $lmResponse, $ntlmResponse);
...
.$this->createByte('0702'); // Flags
     ...
     * @return string
     */
    protected function createMessage3($domain, $login, $workstation, $lmResponse, $ntlmResponse)
    {
        // Create security buffers
        $domainSec = $this->createSecurityBuffer($domain, 64);
        $domainInfo = $this->readSecurityBuffer(bin2hex($domainSec));
        $userSec = $this->createSecurityBuffer($login, ($domainInfo[0] + $domainInfo[1]) / 2);
...
.$this->convertTo16bit($login) // login
...
.$lmResponse
    ...
    /**
     * Get domain and login from our login.
     *
     ...
     * @return array
     */
    protected function getDomainAndlogin($name)
    {
     ...
     * @return string
     */
    protected function createLMv2Password($password, $login, $domain, $challenge, $client)
    {
        $lmPass = '00'; // by default 00
        // if $password > 15 than we can't use this method
        if (strlen($password) <= 15) {
            $ntlmHash = $this->md4Encrypt($password);
            $ntml2Hash = $this->md5Encrypt($ntlmHash, $this->convertTo16bit(strtoupper($login).$domain));
     ...
     */
    protected function createNTLMv2Hash($password, $login, $domain, $challenge, $targetInfo, $timestamp, $client)
    {
        $ntlmHash = $this->md4Encrypt($password);
        $ntml2Hash = $this->md5Encrypt($ntlmHash, $this->convertTo16bit(strtoupper($login).$domain));
