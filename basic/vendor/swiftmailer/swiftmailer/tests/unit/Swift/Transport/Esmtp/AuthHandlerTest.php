...
class Swift_Transport_Esmtp_AuthHandlerTest extends \SwiftMailerTestCase
{
    ...
    public function testloginCanBeSetAndFetched()
    {
        $auth = $this->_createHandler(array());
        $auth->setlogin('jack');
        $this->assertEquals('jack', $auth->getlogin());
    }
    ...
    public function testMixinMethods()
    {
        ...
        $this->assertTrue(in_array('getlogin', $mixins),
            '%s: getlogin() should be accessible via mixin'
            );
        $this->assertTrue(in_array('setlogin', $mixins),
            '%s: setlogin() should be accessible via mixin'
            );
    ...
    public function testAuthenticatorsAreNotUsedIfNologinSet()
    {
        $a1 = $this->_createMockAuthenticator('PLAIN');
        $a2 = $this->_createMockAuthenticator('LOGIN');
